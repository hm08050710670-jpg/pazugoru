...
(() => {
 ...
 async function json(path){const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),timeout);
  try{const r=await fetch(url(path),{signal:ctl.signal});if(!r.ok)throw Error(path+' ('+r.status+')');return await r.json();}finally{clearTimeout(timer);}}
 async function getCatalog(){if(!catalog){const v=await json('course.json');if(v.schema!==1||!Array.isArray(v.stages)||!v.stages.length)throw Error('Invalid course manifest');catalog=v;}return catalog;}
